...
# Check gene expression correlation within infected cohort (libraries 25,26,28) and 
# within control cohort (libraries 27,29,30) to 
...
chart.Correlation(R = log(x = (dgelist_norm$counts[, grep(
  pattern = "S(25|26|28)$", x = colnames(dgelist_norm$counts),
...
png(file = file.path(paste0(imgDir, method, "_Control", "_cor.png", sep = "")),
    width = 1500, height = 1500, units = "px")
chart.Correlation(R = log(x = (dgelist_norm$counts[, grep(
  pattern = "S(27|29|30)$", x = colnames(dgelist_norm$counts),
  perl = TRUE)] + 1), base = 2), histogram = TRUE, method = "spearman",
...
all_coord <- getBCVcoord(dgelist_norm, c("_S25", "_S26", "_S28",
                                         "_S27", "_S29", "_S30"))
...
ggplot(Disp) +
  ...
  theme_bw(base_size = 14, base_family = "Calibri") +
  ggtitle("Estimated dispersions for NEBnext Kit") +
