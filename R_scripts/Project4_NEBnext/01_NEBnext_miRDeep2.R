...
here()
# Output from here():
# [1] "C:/Users/Sarah/Dropbox/@WP2/miRNA_seqLibraykit/miSeq_Bioinformatics/R_scripts/Project4_NEBnext"
...
method <- "NEBnext_miRDeep2"

# Load previously saved data
load("miRNAseq_NEBnext_miRDeep2.RData")
...
colnames(rawCounts) %<>%
  ...
  str_replace("_\\w\\d+_\\w\\d_\\d+_\\w+\\.\\w+", "")
...
# Select counts only
annotCounts %>%
  dplyr::select(starts_with("NEB")) -> counts
...
annotCounts %>%
  dplyr::select(-starts_with("NEB")) -> gene_annot
...
# more than 50 counts per million in 3 or more libraries
# (3 libraries correspond to 3 animals in either group
# (infected or control groups)
dgelist_filt <- dgelist_no_zeros[rowSums(cpm(dgelist_no_zeros) > 50) >= 3, ]
