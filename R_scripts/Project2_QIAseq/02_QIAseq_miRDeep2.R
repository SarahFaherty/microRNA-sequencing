...
load("miRNAseq_QIAseq_miRDeep2.RData")
...
tidy_dgelist$sample %<>%
  factor(levels = c("QIAseq3_S9", "QIAseq3_S10", "QIAseq3_S11", "QIAseq3_S12",
                    "QIAseq3_S13", "QIAseq3_S14", "QIAseq3_S15", "QIAseq3_S16"))
...
# Check gene expression correlation within infected cohort (libraries 9-12) and 
# within control cohort (libraries 13-16) to 
...
chart.Correlation(R = log(x = (dgelist_norm$counts[, grep(
  pattern = "S(9|10|11|12)$", x = colnames(dgelist_norm$counts),
...
png(file = file.path(paste0(imgDir, method, "_Control", "_cor.png", sep = "")),
    width = 1500, height = 1500, units = "px")
chart.Correlation(R = log(x = (dgelist_norm$counts[, grep(
  pattern = "S(13|14|15|16)$", x = colnames(dgelist_norm$counts),
  perl = TRUE)] + 1), base = 2), histogram = TRUE, method = "spearman",
...
getBCVcoord <- function(dgelst, time_pattrn) {
  ...
  mds_coord$group %<>% # Clean group info for plotting
    ...
    str_replace("S(13|14|15|16)", "Control") %>%
    str_replace("S(9|10|11|12)", "Infected")
...
all_coord <- getBCVcoord(dgelist_norm, c("_S9", "_S10", "_S11",
                                         "_S12", "_S13", "_S14", "_S15", "_S16"))
...
ggplot(Disp) +
  ...
  theme_bw(base_size = 14, base_family = "Calibri") +
  ggtitle("Estimated dispersions for QIAseq Kit") +
