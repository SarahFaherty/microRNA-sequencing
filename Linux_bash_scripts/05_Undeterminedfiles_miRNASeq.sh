...
grep Undetermined md5byu.txt > md5byu_Undetermined.txt
wc -l md5byu_Undetermined.txt
cp md5byu_Undetermined.txt Undetermined_files
...
cd $HOME/BTB_SFI_Project/WP2/miRNA_LibraryKit_Study/raw_data/Unaligned/Undetermined_files
awk '{print $2}' md5byu_Undetermined.txt | perl -p -e 's/fslgroup.*(Undetermined.*\.gz)$/$1/' | perl -p -e s'/\///' > byumd5_filenames.txt 
paste <(awk '{print $1}' md5byu_Undetermined.txt) <(awk '{print $1}' byumd5_filenames.txt) > md5byu_Undetermined_corrected.txt 
# Check md5sum files  
md5sum -c md5byu_Undetermined_corrected.txt >> md5_UCD_Undetermined.txt
# Check that all files passed the check:
grep -c 'OK' md5_UCD_Undetermined.txt
